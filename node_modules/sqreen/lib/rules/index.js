...
const ReactiveCallbacks = require('./reactive-callbacks');
...
const instantiateReactive = function (rule) {

    const callbackName = rule.hookpoint.callback_class;
    const callback = ReactiveCallbacks[callbackName];

    if (callback === undefined) {
        throw new Error(`Callback ${callbackName} is unknown`);
    }
    new callback(rule); // garbage collection is prevented through a closure in reactiveCB
};
...
const loadRuleList  = module.exports._loadRuleList = function (ruleList, doNotVerifySignature) {
    ...
    for (let i = 0; i < ruleList.length; ++i) {
        ...
        if (rule.reactive && rule.reactive.authorized_addresses) {
            // This is a reactive rule
            try {
                instantiateReactive(rule);
            }
            catch (err) {
