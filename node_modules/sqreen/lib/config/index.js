...
const configSchema = Joi.object().keys({
    ...
    manifest_updates: Joi.alternatives().try(Joi.string(), Joi.object()).optional().default(DEFAULT.manifest_updates),
    libsqreen: Joi.boolean().truthy('true', 'yes', 1, '1').falsy('false', 'no', 0, '0').default(DEFAULT.libsqreen)
});
...
const parseConfig = function (rawConfig) {
    ...
    if (typeof config.manifest_updates === 'object' && config.manifest_updates !== null) {
        config.manifest_updates = Object.keys(config.manifest_updates)
            .map((k) => `${k}=${config.manifest_updates[k]}`)
            .join(',');
    }
...
const readConfig = function (from) {
    ...
    // backward compatibility for a typo
    if (Array.isArray(config.strip_sensitive_regex)) {
        config.strip_sentitive_regex = config.strip_sensitive_regex;
        delete config.strip_sensitive_regex;
    }
    ...
    if (process.env.SQREEN_MANIFEST_UPDATES) {
        config.manifest_updates = process.env.SQREEN_MANIFEST_UPDATES;
    }

    if (process.env.SQREEN_LIBSQREEN) {
        config.libsqreen = asBoolean(process.env.SQREEN_LIBSQREEN);
    }
...
const getConfig = function (force) {
    ...
    if (!agentConfig || force) {
        if (process.env.SQREEN_DISABLE && asBoolean(process.env.SQREEN_DISABLE)) {
            agentConfig = {
                token: 'none',
                run_in_test: true
            };
