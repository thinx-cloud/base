    ...
    constructor(rule) {
        ...
        this.pre = function (args, value, _rule, selfObj, session) {
            ...
            if ( session.req.__sqreen_xss_lookup) {
                ...
                const claims = Util.getLookableClaims(session.req);
                const spanClaims = Addresses.resolveAllHTTPAddresses(session.topSpan);
                if (spanClaims !== null) {
                    Object.assign(claims, spanClaims);
                }
                const tmp = Flat(claims);
