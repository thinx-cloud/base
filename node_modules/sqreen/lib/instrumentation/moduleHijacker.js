...
const wrappers = InstrumentationInterface.wrappers;
...
module.exports.enable = function () {
    ...
    Module._load = function (request, parent) { // replace the Module._load method
        ...
        try {
            InstrumentationInterface.loader.emit(identity.name, { module: loadedModule, identity, request });
        }
        catch (err) {
            //$lab:coverage:off$
            logErr(err, request);
        ...
        if (wrappers.has(identity.name)) {
            const exports = wrappers.get(identity.name)(loadedModule, identity, request);

            // override the module cache with our wrapped value
            Module._cache[file].exports = exports;
