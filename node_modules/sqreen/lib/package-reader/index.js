...
const MAX_LISTER_RUN = 20000; // 20k runs

const superLister = module.exports.superLister = function (root) {
...
const getPackageForDir = function (path) {
    return new Promise((resolve, reject) => {
        ...
        Fs.readFile(path, (err, content) => {
            ...
            try {
                ...
                resolve(Object.keys(deps).map((name) => `${name}-${deps[name]}`));
            } catch (e) {
                reject(e);
            }
        });
...
module.exports.getDependenciesHash = function (callback) {
    ...
    let res = getPackageForDir(baseDir).catch(() => []);

    if (config.use_workspace === true) {
        for (let i = 0; i < config.workspace_depth; ++i) {
            baseDir = Path.dirname(baseDir);

            res = Promise.all([res, getPackageForDir(baseDir).catch(() => [])])
